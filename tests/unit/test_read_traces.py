...
def test_build_params_includes_filter_when_given():
    params = build_params(
        "1h", limit=10, trace_filter="span:dvc_list_files", now=FIXED_NOW
    )

    assert params["filter"] == "span:dvc_list_files"
    ...
    line = format_trace(
        {
            "traceId": "abc123",
            "spans": [
                {"name": "dvc_list_files", "startTime": "2026-09-07T12:00:00.123Z"}
            ],
        }
    )

    assert "dvc_list_files" in line
...
def test_main_prints_each_returned_trace(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        ...
        lambda project, params: [
            {"traceId": "t1", "spans": [{"name": "dvc_list_files", "startTime": "x"}]},
            {"traceId": "t2", "spans": [{"name": "query_bigquery"}]},
        ],
    )

    assert main([]) == 0
    out = capsys.readouterr().out
    assert "dvc_list_files" in out
    assert "query_bigquery" in out
    ...
    line = format_trace(
        {
            ...
            "spans": [
                ...
                {"name": "execute_tool dvc_list_files"},
                {"name": "dvc_list_files"},
            ],
        },
        spans=True,
...
def test_format_trace_shows_only_the_root_by_default():
    ...
    assert "dvc_list_files" not in line
