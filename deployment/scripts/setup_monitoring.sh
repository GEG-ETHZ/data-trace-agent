#!/usr/bin/env bash
# One-time Cloud Monitoring bootstrap for Data Trace Agent: creates a
...
echo "=== Cloud Monitoring Bootstrap: Data Trace Agent ==="
...
if [ -n "${ALERT_EMAIL:-}" ]; then
  ...
  CHANNEL_IDS+=("$(_find_or_create_channel "Data Trace Agent alerts (email)" \
    --type=email \
...
if [ -n "${SLACK_CHANNEL:-}" ] && [ -n "${SLACK_BOT_TOKEN:-}" ]; then
  ...
  CHANNEL_IDS+=("$(_find_or_create_channel "Data Trace Agent alerts (Slack)" \
    --type=slack \
